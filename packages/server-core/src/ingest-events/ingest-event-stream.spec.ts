...
function makeStream(overrides: Partial<{ findById: unknown; countChapters: unknown }> = {}) {
...
describe('IngestEventStream.open', () => {
  it('404s when the book is missing', async () => {
    const { stream } = makeStream({ findById: jest.fn().mockResolvedValue(null) });
  ...
  it('404s when the book belongs to someone else', async () => {
    const { stream } = makeStream({
      findById: jest.fn().mockResolvedValue(bookRow({ userId: 'someone-else' })),
    });
    await expect(stream.open(BOOK_ID, OWNER)).rejects.toBeInstanceOf(
      ResourceNotFoundException,
    );
  ...
  it('subscribes before reading the seq, so a gap event is not lost', async () => {
    ...
    subscriber.emit({ type: 'stage_entered', bookId: BOOK_ID, seq: 1, stage: 'extracting', status: 'extracting' });
  ...
  it('builds a snapshot carrying the current seq', async () => {
    ...
    expect(session.snapshot).toMatchObject({ type: 'snapshot', seq: 7, stage: 'extracting' });
    session.close();
  });
});
...
describe('IngestStreamSession.run', () => {
  it('writes fresh events and drops any at or below the snapshot seq', async () => {
    ...
    subscriber.emit({ type: 'book_identified', bookId: BOOK_ID, seq: 4, title: 'x', author: null });
    subscriber.emit({ type: 'stage_entered', bookId: BOOK_ID, seq: 6, stage: 'chunking', status: 'chunking' });
  ...
  it('closes the stream on the first terminal event', async () => {
    ...
    subscriber.emit({ type: 'book_completed', bookId: BOOK_ID, seq: 3, status: 'ready' });
    await done;
