...
`packages/worker/prototypes/chapter-detection/regen.mjs` turns a real LlamaParse JSON capture plus its PDF into a `*.detection-input.json`.
See that script's header for usage.

The full _Pragmatic Programmer_ capture is intentionally not committed here yet.
It needs the licensed PDF and a `LLAMAPARSE_API_KEY`.
Drop `pragmatic-programmer.detection-input.json` and its `.expected.json` in once captured and the spec picks it up automatically.
