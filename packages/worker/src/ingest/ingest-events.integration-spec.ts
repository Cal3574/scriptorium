...
describe('live ingest progress: pipeline -> SSE bridge (Seam 2)', () => {
    ...
    return {
      ...
      events(): IngestEvent[] {
        return frames
          ...
          .map((f) => JSON.parse(/data: (.*)/.exec(f)?.[1] ?? '{}') as IngestEvent);
      },
  ...
  it('streams a snapshot, then the pipeline stage events in order with monotonic seq', async () => {
    ...
    expect(session.snapshot).toMatchObject({ type: 'snapshot', status: 'pending', seq: 0 });
  ...
  it('forwards a terminal book_failed event and closes the stream', async () => {
    ...
    const brokenExtractor: PdfExtractor = {
      extract: () => Promise.reject(new PdfExtractionError('PDF_IS_BROKEN', false)),
    };
    await expect(makeProcessor(brokenExtractor).process(bookId)).rejects.toThrow(
      /PDF_IS_BROKEN/,
    );
