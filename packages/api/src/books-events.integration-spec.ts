...
describe('live ingest progress over SSE', () => {
  ...
  async function seedBook(opts: { ownerClerkId?: string; status?: string } = {}) {
    const [owner] = (
      await db.pool.query(
    ...
    await db.pool.query(
      ...
      [bookId, owner.id, `books/${owner.id}/${bookId}.pdf`, opts.status ?? 'pending'],
    );
    return bookId;
  }
  ...
  async function openStream(bookId: string, token: string): Promise<{
    status: number;
  ...
  function requireStream(opened: { status: number; stream?: OpenStream }): OpenStream {
    if (!opened.stream) {
  ...
  it('leads with a snapshot built from the DB row, then streams deltas in seq order', async () => {
    ...
    expect(snapshot).toMatchObject({ type: 'snapshot', status: 'extracting', seq: 0 });

    await publish(bookId, { type: 'stage_entered', stage: 'extracting', status: 'extracting' });
  ...
  it('drops events already covered by the snapshot after a reconnect', async () => {
    ...
    await publish(bookId, { type: 'stage_entered', stage: 'extracting', status: 'extracting' });
    await publish(bookId, { type: 'book_identified', title: 'T', author: null });
    ...
    await publish(bookId, { type: 'stage_progress', stage: 'embedding', done: 1, total: 4, unit: 'chunks' });
    const progress = await s.waitFor('stage_progress');
    expect(progress.seq).toBe(3);
    // Nothing with seq <= 2 was replayed.
    expect(s.events().every((e) => e.event === 'snapshot' || e.data.seq > 2)).toBe(true);
    s.close();
  });
