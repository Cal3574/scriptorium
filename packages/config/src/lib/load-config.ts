...
// Attach to every process schema: in `live` mode each required provider key
// must be a non-empty string, reported against its own key so the
...
function requireProviderKeysWhenLive(
  ...
  for (const key of alwaysLiveKeys) {
    if (!cfg[key]) {
      ctx.addIssue({
